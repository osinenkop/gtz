...
four-parameter structured ansatz.  Modular lex computations and CRT
reconstruction reduce the relevant branch to a degree-eleven candidate
eliminant in the margin parameter; numerical classification of its real roots
finds no point satisfying both active and inactive inequalities.
...
On this branch, modular lex computations over six primes have stable quotient
degree \(184\) and a degree-\(22\) eliminant in the tie parameter \(q\), where
the tied outside triples have least eigenvalue \(q/6\).  The visible factor
\[
...
A deterministic numerical classifier using these lifted \(q\)-values found
\(40\) real ansatz roots in two independent runs.  Twenty pass active positive
semidefiniteness, but none pass the inactive inequalities.  The active-PSD roots
occur only at \(q=1.180695836766\ldots\) and \(q=5\), and both cases have
positive inactive overshoot.  The smaller real \(q\)-values fail active
positive semidefiniteness.

\begin{remark}
This subsection is evidence, not a proof.  The remaining exact step is to
certify the reconstructed \(q\)-factor by a characteristic-zero normal-form or
membership computation, then isolate the real roots and prove the indicated sign
conditions with rational interval arithmetic.
\end{remark}
