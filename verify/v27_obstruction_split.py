...
def diagnose(v, active):
    ...
    if inact:
        inactive_witness = max(inact, key=lambda i: float(lam[i] - TH))
        excess = float(lam[inactive_witness] - TH)
    else:
    ...
    return dict(worst_active_minor=worst_minor, inactive_excess=excess,
                inactive_witness=(None if inactive_witness is None else int(inactive_witness)),
                inactive_triple=(None if inactive_witness is None else list(TRIPLES[inactive_witness])),
                inactive_lambda=(None if inactive_witness is None else float(lam[inactive_witness])),
                actual_active=actual_active,
                actual_active_triples=[list(TRIPLES[i]) for i in actual_active],
                actual_size=int(bk), gap_ratio=float(br),
                F=float(np.max(lam)))


def probe(indices, n_starts, seed, maxiter):
...
def write_output(path, sizes, starts, seed, maxiter, kind, rows, complete):
    out_dir = os.path.dirname(path)
    if out_dir:
        os.makedirs(out_dir, exist_ok=True)
    json.dump(dict(sizes=sizes, starts=starts, seed=seed, maxiter=maxiter,
                   complete=complete, classification=kind, rows=rows),
...
if __name__ == "__main__":
    ...
    ap.add_argument("--canons", default="")
    ap.add_argument("--out", default="verify/out/v27_obstruction_split.json")
    a = ap.parse_args()
    ...
    if a.canons:
        canons = {int(x) for x in a.canons.split(",") if x}
        orbits = [x for x in orbits if x["canon"] in canons]
    if a.offset:
        orbits = orbits[a.offset:]
    if a.limit:
        orbits = orbits[:a.limit]

    print("=" * 78)
    print("OBSTRUCTION SPLIT: why are the low-active strata empty?")
    print(f"  sizes={sizes}  orbits={len(orbits)}  starts={a.starts}  maxiter={a.maxiter}")
    ...
    for n, orb in enumerate(orbits, 1):
        ...
        br, onloc = probe(idx, a.starts, a.seed + 99991 * n, a.maxiter)
        if not onloc:
            kind["off"] += 1
            rows.append(dict(size=orb["size"], canon=orb["canon"],
                             input_indices=idx,
                             input_triples=[list(t) for t in orb["triples"]],
                             on_locus=0,
                             best_eq_residual=float(br)))
            write_output(a.out, sizes, a.starts, a.seed, a.maxiter,
                         kind, rows, complete=False)
